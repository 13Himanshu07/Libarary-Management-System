...
├── sql/
│   └── library_db.sql        # run once to create the database + tables (+ sample data)
├── src/com/library/
...
```
This creates the `library_db` database, its three tables (`books`,
...
- The loan period defaults to 14 days and the late fine is ₹5/day — both
  are constants (`DEFAULT_LOAN_DAYS`, `FINE_PER_DAY`) at the top of
